...
ulboraCmsLinkControllers.controller('NewLinkCtrl', ['$scope', 'checkCreds', '$location', '$http', 'getToken', 'ArticleValues',
    function NewLinkCtrl($scope, checkCreds, $location, $http, getToken, ArticleValues) {
        if (checkCreds() !== true) {
            $location.path('/loginForm');
        }
        $http.defaults.headers.common['Authorization'] = 'Basic ' + getToken();
