...
ulboraCmsUserControllers.controller('UserCtrl', ['$scope', 'checkCreds', '$location', "User", '$routeParams', '$http', 'getToken',
    function UserCtrl($scope, checkCreds, $location, User, $routeParams, $http, getToken) {
        if (checkCreds() !== true) {
            $location.path('/loginForm');
        }
        $http.defaults.headers.common['Authorization'] = 'Basic ' + getToken();
        ...
        function success(response) {
            ...
            console.log("Success:" + JSON.stringify(response));
            var userEnabled = "false";
            if (response.user.enabled === true) {
                userEnabled = "true";
            }

            $scope.id = response.id;
            $scope.username = response.user.username;
            $scope.username2 = response.user.username;
            ...
            $scope.roleId = response.user.role.id;
            $scope.userRoleList = response.roleList;
